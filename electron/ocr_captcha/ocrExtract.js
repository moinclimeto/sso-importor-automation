    ...
    switch (companyDocType) {
      case 'auto':
        return `OCR Company Document. IDENTIFY the document type and extract accordingly. JSON only minified. 
          ...
          GST vs UNIT_GST: Use doc_type=gst for principal/registered office GST certificate. Use doc_type=unit_gst for plant/factory/unit/additional place of business GST certificate (usually different GSTIN or different address).
      ...
      case 'gst':
        return `OCR GST Certificate. JSON only minified. Extract: {"doc_type":"gst","document_number":"","entity_name":"","trade_name":"","constitution_of_business":"","address":"","district":"","issue_date":"YYYY-MM-DD","date_of_liability":"YYYY-MM-DD"}. RULES: document_number=GSTIN. entity_name=Legal Name. trade_name=Trade Name. issue_date=Date of Registration. address=clean comma-separated postal address WITHOUT labels like "Building No./Flat No.:" or "District:". district=district name only (e.g. Raisen).${fileHint}`;
      case 'unit_gst':
        return `OCR Unit/Plant GST Certificate. JSON only minified. Extract: {"doc_type":"unit_gst","document_number":"","entity_name":"","trade_name":"","constitution_of_business":"","address":"","district":"","issue_date":"YYYY-MM-DD","date_of_liability":"YYYY-MM-DD"}. RULES: document_number=Unit GSTIN. entity_name=Legal Name. address=Plant/Unit/factory address (clean comma-separated, no labels). district=district name only.${fileHint}`;
      case 'pan':
