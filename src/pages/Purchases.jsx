...
export default function Purchases() {
  ...
  const load = async () => {
    ...
    const [data, comps] = await Promise.all([
      window.pwp.purchases.getAll(activeFilters),
  ...
  const handleSave = async (form) => {
    ...
    if (modal?.id) await window.pwp.purchases.update({ ...form, id: modal.id });
    else await window.pwp.purchases.add(form);
    setModal(null);
    load();
  };
  ...
  const handleDelete = async (id) => {
    ...
    await window.pwp.purchases.delete(id);
    load();
  };
            ...
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody>
                {records.map((r, i) => (
                  <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="td text-right font-semibold text-blue-600">₹{fmt(r.total_amount)}</td>
              ...
              <tfoot>
                <tr className="bg-blue-50 border-t-2 border-blue-200">
                  ...
                  <td className="td font-bold text-blue-700 text-right text-base">₹{fmt(totalAmount)}</td>
                  <td className="td" />
                </tr>
              </tfoot>
            </table>
