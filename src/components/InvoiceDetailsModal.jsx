...
export default function InvoiceDetailsModal({ open, invoice, docType = 'purchase', onClose }) {
  ...
  const fileName =
    ...
    invoice.fileName || data.invoice_filename || data.invoice_file_name || '—';

  const invoiceNo =

    data.invoice_number || data.application_number || data.invoice_no || '—';
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
      ...
      <div
        ...
        className="w-full max-w-5xl relative max-h-[90vh] flex flex-col rounded-2xl border border-slate-200 bg-white shadow-xl"
      ...
      >
        ...
        <div className="flex items-start gap-2 px-5 pt-5 pb-3 pr-12 border-b border-slate-100">
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1 min-h-0 space-y-4">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-3">
                    ...
                    <tbody>
                      ...
                      {lineItems.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="td">{displayValue(item.plasticMaterial)}</td>

                          <td className="td">{displayValue(item.plasticCategory)}</td>

                          <td className="td">{displayValue(item.recycledPercent)}</td>
                          ...
                          <td className="td text-right tabular-nums">
                            ...
                            {displayAmount(item.gstAmount)}

                          </td>

                          <td className="td text-right tabular-nums">

                            {displayAmount(item.gstRate)}
